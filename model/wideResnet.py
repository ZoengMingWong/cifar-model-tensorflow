...
def dropoutBlock(in_feat, stride, out_chans, is_training, block_id, weight_decay=1e-4):
    ...
    dropout = tf.layers.dropout(relu1, 0.3, training=is_training)
    
    conv2 = layers.conv2d(dropout, out_chans, kernel_size=[3, 3], strides=[1, 1], padding='SAME', 
                          activation=None, use_bias=False, weight_decay=weight_decay, name='Block_'+str(block_id)+'_conv2')
...
def WideResNet(img, blocks, strides, k, chans, is_training, weight_decay=1e-4, dropout=True):
    ...
    for i in range(len(stride)):
        ...
        if dropout == False:
            ...
            resBlock.append(dropoutBlock(resBlock[-1], stride[i], chans, is_training, block_id=i, weight_decay=weight_decay))
...
def WRN_40_2(img, is_training, weight_decay=1e-4):
    return WideResNet(img, [6, 6, 6], [1, 2, 2], 2, 16, is_training, weight_decay)

def WRN_16_4(img, is_training, weight_decay=1e-4):
    return WideResNet(img, [2, 2, 2], [1, 2, 2], 4, 16, is_training, weight_decay)

def WRN_28_10(img, is_training, weight_decay=1e-4):
    return WideResNet(img, [4, 4, 4], [1, 2, 2], 10, 16, is_training, weight_decay)
